...
def wait_until_listing_ready(session, api_key, listing_id, max_wait_seconds=25):
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Accept-Version": "3.0",
    }
    ...
    while time.time() - start < max_wait_seconds:
        try:
            check_response = session.get(f"{API_BASE}/listings/{listing_id}", headers=headers, timeout=10)
...
def upload_images(session, api_key, listing_id, image_paths):
    """Upload images with retries and MIME handling."""
    if not image_paths:
    ...
    endpoint = f"{API_BASE}/listings/{listing_id}/photos"
    progress_bar = st.progress(0)
    status_text = st.empty()
        ...
        try:
            ...
            for attempt in range(1, 4):
                with open(image_path, "rb") as img_file:
                    files = {
                        "photo": (Path(image_path).name, img_file, mime_type),
                    }
